...
#B ) A’da üretilen matriste bir leri içeren mbr dikdörtgenini üreten fonksiyonu yazınız.a da elde edilen matris içerisinde 1 leri içeren MBR i return eden
...
#def matris_create_random(x = 28, y = 28):
...
def get_similarity(character_a,character_b):
    n = character_a.shape[0]
    m = character_a.shape[1]
    my_similarity = 0
    for i in range(n):
        for j in range(m):
            my_similarity += character_a[i,j]*character_b[i,j]
